...
public class ConvertedFieldBuilder<T, S>
{
	...
	public FieldModel<T> using(Converter<T, S> converter)
	{
		...
		{
			return formModel.createFieldModel(convert((MutableValueModel<S>) from).using(converter), valueType);
		}
		else
		{
			return formModel.createFieldModel(convert(from).using(converter), valueType);
		}
	}
}
