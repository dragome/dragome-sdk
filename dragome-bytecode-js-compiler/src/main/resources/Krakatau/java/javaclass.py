...
def generateAST(cls, cb, skip_errors, method=None, add_throws=False):
    ...
    matches = [m for m in cls.methods if (m.name.encode('utf8')+ m.descriptor.encode('utf8')) == method]
    ...
    return ast2.ClassDef('', False, cls.name, superc, interfaces, field_defs, method_defs)
